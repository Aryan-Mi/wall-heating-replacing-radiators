...
def load_reference_timing(ref_csv: Path) -> float | None:
    """Return seconds_per_floorplan from the Task 2 timing CSV, or None."""
    try:
...
def time_simulation(n_floorplans: int, data_dir: str) -> tuple[float, float]:
    """Run simulate_numba_jit_cpu.py and return (wall_seconds, internal_seconds)."""
    script = Path(__file__).parent / "simulate_numba_jit_cpu.py"
...
def count_all_floorplans(data_dir: str) -> int:
    """Count total available floorplans in data_dir."""
...
def plot_comparison(output_path: Path, numba_spf: float, ref_spf: float | None) -> None:
    mpl.use("Agg")
    labels = ["Numba JIT (Task 7)"]
    values = [numba_spf]
    if ref_spf is not None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Task 7: benchmark Numba JIT CPU Jacobi")
    ...
    parser.add_argument("--output-dir", default="outputs/07-numba-jit-cpu")
    parser.add_argument(
        "--reference-csv",
