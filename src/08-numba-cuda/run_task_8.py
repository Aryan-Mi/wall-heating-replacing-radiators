...
def plot_comparison(output_path: Path, cuda_spf: float, ref_spf: float | None) -> None:
    mpl.use("Agg")
    labels = ["Numba CUDA (Task 8)"]
    values = [cuda_spf]
    if ref_spf is not None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Task 8: benchmark Numba CUDA Jacobi")
    ...
    parser.add_argument("--output-dir", default="outputs/08-numba-cuda")
    parser.add_argument(
        "--reference-csv",
