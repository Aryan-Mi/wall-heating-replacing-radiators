...
def load_data(load_dir, bid):
    ...
    u = np.zeros((SIZE + 2, SIZE + 2)) # Pre-pad the floor plan with a 1-pixel border of zeros to simplify jacobi iterations (no need for boundary checks)
    u[1:-1, 1:-1] = np.load(join(load_dir, f"{bid}_domain.npy")) # Overlay the domain image on u. U is now the padded domain image
...
def jacobi(u, interior_mask, max_iter, atol=1e-6):
    """
        ...
        Step 1 updates the temperature at each interior point to be the average of its four neighbors (left, right, up, down).
    ...
    for _ in range(max_iter):
        ...
        u_new_interior = u_new[interior_mask] # Only consider the interior points for the stopping criterion and update. The boundary points (walls) remain fixed.
...
def summary_stats(u, interior_mask):
    """
        Computes summary statistics for the interior temperatures after the simulation has converged.
...
if __name__ == "__main__":
    ...
    all_u = np.empty_like(all_u0) # all_u shares the same shape as all_u0, but will store the updated floor plan domains after Jacobi iterations.
